...
class _MenuScreenState extends State<MenuScreen> {
  ...
  final Map<String, List<MenuItem>> _categorias = {
    'Pizzas': [
      const MenuItem(
        nombre: 'Pizza Cañon',
        descripcion:
  ...
  List<MapEntry<String, List<MenuItem>>> get _categoriasVisibles {
    if (_filtroActivo == 'Todas') return _categorias.entries.toList();
    return _categorias.entries.where((e) => e.key == _filtroActivo).toList();
  }
        ...
        itemBuilder: (context, i) {
          ...
          return GestureDetector(
            onTap: () {
              ...
              _abrirCategoria(filtro, _categorias[filtro] ?? []);
            },
            child: Container(
        ...
        for (final result in resultados)
          Padding(
            ...
            child: _MenuCard(
              item: result.value,
              onTap: () =>
                  _abrirCategoria(result.key, _categorias[result.key] ?? []),
            ),
          ),
      ],
    );
